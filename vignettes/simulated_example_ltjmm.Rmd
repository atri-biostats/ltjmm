...
# Fit LTJMM with no random effects with Stan
```{r stanFitNone, eval=FALSE}
fit <- ltjmm_stan(Y ~ year |
    1  | # fixed effects direct on outcome
    id | outcome,
  random_effects = "none",
  ...
  chains = 2, parallel_chains = 2,
  iter_warmup = 500, iter_sampling = 500, thin = 2)
# fit$save_object(file = 'sim_lt_no_ranef_results.RDS')
fit.sum <- fit$summary(c("beta", "gamma", "sigma_delta", "delta")) %>%
  as.data.frame()
row.names(fit.sum) <- fit.sum %>% pull(variable)
draws.beta <- fit$draws("beta")
draws.gamma <- fit$draws("gamma")
draws.sigma_delta <- fit$draws("sigma_delta")
save(fit.sum, draws.beta, draws.gamma, draws.sigma_delta, 
  file = 'sim_lt_no_ranef_results.rdata')
```

```{r loadResultsNone}
load(system.file("testdata", "sim_lt_no_ranef_results.rdata", package="ltjmm"))
```
